...
def _ordinal_soft_targets(y, num_classes, sigma):
    ...
    return F.softmax(-dist_sq / (2.0 * sigma * sigma), dim=1)


def run_epoch(model, loader, optimizer=None, device="cpu",
              label_smoothing=0.0, grad_clip=1.0, ordinal_sigma=0.0):
    ...
    for x, y, _ in loader:
        ...
        logits = model(x)
        if ordinal_sigma > 0.0 and logits.size(-1) >= 3:
            soft = _ordinal_soft_targets(y, num_classes=logits.size(-1),
                                         sigma=ordinal_sigma)
...
def main():
    ...
    ap.add_argument("--context_seconds", type=float, default=2.0,
                    help="Audio context duration fed to AST. 2.0 = original single-segment "
                         "behavior. >2.0 triggers stitching of consecutive segments; 10.0 "
                         "matches the HF AST pretraining length and uses the full receptive field.")
    ...
    if args.eval:
        ...
        if ds._stitch_enabled:
            print("[INFO] Prestitching groups before DataLoader workers spawn...",
                  flush=True)
            ds.prestitch_groups(verbose=True)
    ...
    # Prestitch groups up-front so DataLoader workers only ever read from cache.
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if score > best_score:
            ...
            try:
                ...
                # Disk-full on Kaggle: purge the per-clip AST feature cache
                # (top-level .pt files in cache_dir; preserves stitched_*/ subdir)
                # and retry once.
                print(f"  [WARN] Checkpoint save failed ({e}); purging AST "
                      f"feature cache and retrying.")
                try:
                ...
                if args.cache_dir:
                    ...
                    for p in cdir.glob("*.tmp"):
                        try:
                            ...
                            pass
                    print(f"  [WARN] Purged {purged} cache files from {cdir}")
                torch.save(ckpt, out_path)
