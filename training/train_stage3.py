...
def main():
    ...
    ap.add_argument("--min_quality", type=int, default=1,
                    help="Keep only clips with quality >= this (default 1, all samples)")
    ...
    model_mod = load_model_class("3_stages_cnn_v1")
    ThreeStagesCNNv1 = model_mod.ThreeStagesCNNv1
    ...
    if args.eval:
        ...
        model.eval()
        ds = AudioDataset(args.test_csv, task="stage3", cfg=cfg, min_quality=mq, dataset_root=args.dataset_root)
    ...
    val_ds = AudioDataset(args.val_csv, task="stage3", cfg=cfg, min_quality=mq, dataset_root=args.dataset_root)
    ...
    if args.use_weighted_sampler:
        sampler, counts = make_weighted_sampler_stage3_from_csv(args.train_csv, min_quality=mq)
