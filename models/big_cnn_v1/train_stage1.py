# models/big_cnn_v1/train_stage1.py
"""
Train BigCNNv1 stage 1 — binary drone detection (drone / no_drone).

The stage-1 backbone is SmallCNNv2 (~80K params), identical to the one used
in 3_stages_cnn_v1.  The "big" upgrade lives entirely in stage 2.

Usage (from project root):
    python models/big_cnn_v1/train_stage1.py \
        --train_csv ml/splits/train.csv \
        --val_csv   ml/splits/val.csv \
        --use_weighted_sampler \
        --out artifacts/checkpoints/stage1_bigcnnv1.pt
"""
...
# Load SmallCNNv2 from sibling v2 directory
...
def run_eval(checkpoint_path: str, csv_path: str, device: str, batch: int, num_workers: int):
    ...
    model = SmallCNNv2(num_classes=2, dropout=0.0).to(device)
...
def main():
    ...
    ap.add_argument("--out", default="artifacts/checkpoints/stage1_bigcnnv1.pt")
    ...
    set_seed(args.seed)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[INFO] device={device}  model=SmallCNNv2 (BigCNNv1 stage1)  dropout={args.dropout}")
