# models/small_cnn_v3/train_stage1.py
"""
Train SmallCNNv3 stage 1 (drone detection, binary).

Key improvement over v2: waveform-level noise mixing during training.
Drone samples are randomly overlaid with not_a_drone audio (speech, wind,
cars, etc.) at varying SNRs, forcing the model to learn noise-invariant
features.  Speech and wind are sampled 3x more often since they are the
most common real-world contaminants in drone recordings.

Also includes: gain jitter, SpecAugment, label smoothing, AdamW + cosine LR.

Usage (from project root):
    python models/small_cnn_v3/train_stage1.py \
        --train_csv ml/splits/train.csv \
        --val_csv   ml/splits/val.csv \
        --use_weighted_sampler \
        --out artifacts/checkpoints/stage1_smallcnnv3.pt
"""
...
def main():
    ...
    ap.add_argument("--out", default="artifacts/checkpoints/stage1_smallcnnv3.pt")
    ...
    # SpecAugment (on mel spectrogram, during training)
    spec_augment = []
    if args.freq_mask > 0:
    ...
    model = SmallCNNv3(num_classes=2, dropout=args.dropout).to(device)
